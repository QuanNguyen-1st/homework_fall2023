...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        if self.critic is not None:
            ...
            for _ in range(self.baseline_gradient_steps):
                ...
                critic_losses.append(critic_loss["Baseline Loss"])
            critic_info = { "Critic Loss": np.mean(critic_losses) }           
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    delta = rewards[i] + (1 - terminals[i]) * self.gamma * values[i + 1] - values[i] # δ_t = r_t + γ * V(s_t+1) - V(s_t)
                    advantages[i] = delta + (1 - terminals[i]) * self.gamma * self.gae_lambda * advantages[i + 1] # A_t = δ_t + (γλ)A_t+1
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        involve t)!
        """
        result : Sequence[float] = [0] * len(rewards)
