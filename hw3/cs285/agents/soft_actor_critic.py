...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            next_qs = torch.min(next_qs, dim=0, keepdim=True).values.repeat(next_qs.size(0), 1)
        elif self.target_critic_backup_type == "mean":
            next_qs = torch.mean(next_qs, dim=0, keepdim=True).repeat(next_qs.size(0), 1)
        elif self.target_critic_backup_type == "redq":
            indices = torch.randperm(next_qs.size(0))[:2]
            random_next_qs = next_qs[indices]
            next_qs = torch.min(random_next_qs, dim=0, keepdim=True).values.repeat(next_qs.size(0), 1)
        else:
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        return -action_distribution.log_prob(action_distribution.rsample((self.num_actor_samples,))).mean() if self.actor_gradient_type == "reparametrize" \
            else -action_distribution.log_prob(action_distribution.sample((self.num_actor_samples,))).mean()
