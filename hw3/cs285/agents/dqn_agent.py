...
class DQNAgent(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_q_values = next_qa_values.gather(1, next_action.long().unsqueeze(1))
            target_values = reward.unsqueeze(1) + (self.discount * (1 - done.float())).unsqueeze(1) * next_q_values

        # TODO(student): train the critic with the target values
        qa_values : torch.Tensor = self.critic(obs)
        q_values = qa_values.gather(1, action.long().unsqueeze(1)) # Compute from the data actions; see torch.gather
